    ...
    <BaseDialog isVisible={props.isVisible} onClose={props.onClose} isLockingScroll>
      <div style={{ width: 400 }}>
        ...
        >
          {props.args && (
            <div>
              <Title size={TitleSize.Large} variant={TitleVariant.Highlighted} hasBottomSpacing isCentered>
                Cloud Sync
              </Title>
              <Title size={TitleSize.Medium} hasBottomSpacing>
                It appears that the local version of the project is not up to date. Would you like to overwrite the
                local version with the latest cloud version?
              </Title>
            </div>
          )}
          ...
          {props.args && props.args.action === 'save' && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '24px' }}>
              ...
              <PrimaryButton
                variant={PrimaryButtonVariant.Muted}
                label="Save this version to cloud"
                onClick={async () => {
